...
class ClientStripeAPI {
  // Connect Stripe Account
  async connectAccount(): Promise<StripeConnectResponse> {
    try {
      const response = await clientApiClient.post(API_CONFIG.ENDPOINTS.CLIENT_STRIPE.CONNECT);
      ...
      console.error('Connect Client Stripe account error:', error);
  ...
  async getAccountStatus(): Promise<StripeAccountStatus> {
    try {
      const response = await clientApiClient.get(API_CONFIG.ENDPOINTS.CLIENT_STRIPE.ACCOUNT_STATUS);
      ...
      console.error('Get Client Stripe account status error:', error);
  ...
  async disconnectAccount(): Promise<void> {
    try {
      const response = await clientApiClient.delete(API_CONFIG.ENDPOINTS.CLIENT_STRIPE.DISCONNECT);
      
      if (response.data.error) {
        throw new Error(response.data.message || 'Failed to disconnect Stripe account');
      }
    } catch (error: any) {
      console.error('Disconnect Client Stripe account error:', error);
  ...
  redirectToStripeConnect(): void {
    this.connectAccount()
      ...
      .catch((error) => {
        console.error('Failed to redirect to Client Stripe Connect:', error);
        throw error;
      });
  }
}

export default new ClientStripeAPI();
